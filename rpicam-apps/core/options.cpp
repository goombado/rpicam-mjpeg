...
void Options::ReconstructArgs(std::vector<std::string> &args) const
{
	args.push_back("rpicam");

	if (!config_file.empty())
		args.push_back("--config=" + config_file);
	if (verbose)
		args.push_back("--verbose=" + std::to_string(verbose));
	if (!info_text.empty())
		args.push_back("--info-text=" + info_text);
	if (!timeout_.empty())
		args.push_back("--timeout=" + timeout_);
	if (width)
		args.push_back("--width=" + std::to_string(width));
	if (height)
		args.push_back("--height=" + std::to_string(height));
	if (!output.empty())
		args.push_back("--output=" + output);
	if (!post_process_file.empty())
		args.push_back("--post-process-file=" + post_process_file);
	if (!post_process_libs.empty())
		args.push_back("--post-process-libs=" + post_process_libs);
	if (nopreview)
		args.push_back("--nopreview");
	if (fullscreen)
		args.push_back("--fullscreen");
	if (preview_width || preview_height)
		args.push_back("--preview=" + std::to_string(preview_x) + "," + std::to_string(preview_y) + "," +
					   std::to_string(preview_width) + "," + std::to_string(preview_height));
	if (qt_preview)
		args.push_back("--qt-preview");
	if (hflip_)
		args.push_back("--hflip");
	if (vflip_)
		args.push_back("--vflip");
	if (rotation_)
		args.push_back("--rotation=" + std::to_string(rotation_));
	if (roi_width || roi_height)
		args.push_back("--roi=" + std::to_string(roi_x) + "," + std::to_string(roi_y) + "," +
					   std::to_string(roi_width) + "," + std::to_string(roi_height));
	if (shutter)
		args.push_back("--shutter=" + shutter_);
	if (gain)
		args.push_back("--gain=" + std::to_string(gain));
	if (metering != "centre")
		args.push_back("--metering=" + metering);
	if (exposure != "normal")
		args.push_back("--exposure=" + exposure);
	if (ev)
		args.push_back("--ev=" + std::to_string(ev));
	if (awb != "auto")
		args.push_back("--awb=" + awb);
	if (awb_gain_r || awb_gain_b)
		args.push_back("--awbgains=" + std::to_string(awb_gain_r) + "," + std::to_string(awb_gain_b));
	if (flush)
		args.push_back("--flush");
	if (wrap)
		args.push_back("--wrap=" + std::to_string(wrap));
	if (brightness)
		args.push_back("--brightness=" + std::to_string(brightness));
	if (contrast)
		args.push_back("--contrast=" + std::to_string(contrast));
	if (saturation)
		args.push_back("--saturation=" + std::to_string(saturation));
	if (sharpness)
		args.push_back("--sharpness=" + std::to_string(sharpness));
	if (framerate)
		args.push_back("--framerate=" + std::to_string(framerate.value()));
	if (!denoise.empty())
		args.push_back("--denoise=" + denoise);
	if (viewfinder_width)
		args.push_back("--viewfinder-width=" + std::to_string(viewfinder_width));
	if (viewfinder_height)
		args.push_back("--viewfinder-height=" + std::to_string(viewfinder_height));
	if (tuning_file != "-")
		args.push_back("--tuning-file=" + tuning_file);
	if (lores_width)
		args.push_back("--lores-width=" + std::to_string(lores_width));
	if (lores_height)
		args.push_back("--lores-height=" + std::to_string(lores_height));
	if (!mode_string.empty())
		args.push_back("--mode=" + mode_string);
	if (!viewfinder_mode_string.empty())
		args.push_back("--viewfinder-mode=" + viewfinder_mode_string);
	if (buffer_count)
		args.push_back("--buffer-count=" + std::to_string(buffer_count));
	if (viewfinder_buffer_count)
		args.push_back("--viewfinder-buffer-count=" + std::to_string(viewfinder_buffer_count));
	if (no_raw)
		args.push_back("--no-raw");
	if (afMode_index != -1)
		args.push_back("--autofocus-mode=" + afMode);
	if (afRange_index != -1)
		args.push_back("--autofocus-range=" + afRange);
	if (afSpeed_index != -1)
		args.push_back("--autofocus-speed=" + afSpeed);
	if (afWindow_width || afWindow_height)
		args.push_back("--autofocus-window=" + std::to_string(afWindow_x) + "," + std::to_string(afWindow_y) + "," +
					   std::to_string(afWindow_width) + "," + std::to_string(afWindow_height));
	if (!lens_position_.empty())
		args.push_back("--lens-position=" + lens_position_);
	if (hdr != "off")
		args.push_back("--hdr=" + hdr);
	if (!metadata.empty())
		args.push_back("--metadata=" + metadata);
	if (metadata_format != "json")
		args.push_back("--metadata-format=" + metadata_format);
	if (flicker_period)
		args.push_back("--flicker-period=" + flicker_period_);
